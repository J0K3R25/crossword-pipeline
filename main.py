...
# checks if each word in the crossword contains at least 3 letter 
...
def ask_size():
    while True:
        size_crossword = input("What is the size of the crossword? (Number of Rows/Columns) \n")
        
        # Check if input is valid
        if size_crossword.isdigit() and int(size_crossword) > 0:
            size_crossword = int(size_crossword)
            break 
        else:
            print("Please enter a valid crossword size.")
    return size_crossword
...
ask_print()
#sheet.update(sheet.batch_clear, def_cell_range(0,0,size_crossword,size_crossword))




'''
# The code below shows how to run the solver on a .puz file
puzzle_file = './puzzles/Mar2321.puz'

with open(puzzle_file, "r") as f:
    print('Running solver on file:', f)
    puzzle = Utils.convert_puz(puzzle_file)

crossword_test = Crossword(puzzle)

print(puzzle)

solve(crossword_test)
'''
